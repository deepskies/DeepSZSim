...
class GenerateCluster():
    ...
    def f_sz(self, freq_ghz, T_CMB_K, *args, **kwargs):
        '''
        Parameters:
        ----------
        freq_ghz: float
...
# 3) Convolve submap with beam
    ...
    def convolve_map_with_gaussian_beam(self, pix_size_arcmin, 
                                        beam_size_fwhp_arcmin, map):
        '''
        Parameters:
        ----------
        pix_size_arcmin: float
    ...
    def add_cmb_map_and_convolve(self, dT_map_uK, ps, pix_size_armin, 
        ...
        cmb_map = self.make_cmb_map(shape=expanded_shape, 
                                    pix_size_armin=pix_size_arcmin, ps=ps)
        ...
        conv_map = self.convolve_map_with_gaussian_beam(
...
# 5) Generate noise map
    ...
    def make_cmb_map(self, shape, pix_size_arcmin, ps):
        '''
        Makes a cmb temperature map based on the given power spectrum
...
#other helpful functions
    ...
    def get_tSZ_signal_aperture_photometry(self, Map, radmax_arcmin, 
        ...
        disk_mean = Map[r < radmax_arcmin].mean() #average in inner radius
        ring_mean = Map[(r >= radmax_arcmin) & (r < radius_out)].mean() #average in outer radius
